        ...
        try {
            ...
            if (writeClient && Object.keys(cleanedLabels).length > 0) {
                try {
                    ...
                    const syncResult = await zoneWriter.syncZoneLabelsToHomeAssistant(cleanedLabels, deviceId);
                    if (!syncResult.ok) {
                        logger_1.logger.warn({ deviceId, failures: syncResult.failures }, 'Some zone labels failed to sync to Home Assistant');
                    ...
                    logger_1.logger.info({ deviceId }, 'Zone labels synced to Home Assistant successfully');
                }
                catch (syncError) {
                    logger_1.logger.error({ syncError, deviceId }, 'Failed to sync zone labels to Home Assistant');
